...
void task9() {
    ...
    if (!inputFile1 || inputFile1->IsZombie()) {
        std::cerr << "Ошибка при открытии файла filtered_results.root" << std::endl;
        return;
    }
    ...
    if (!outputTree) {
        std::cerr << "Дерево MyTree не найдено в newroot.root" << std::endl;
        inputFile1->Close();
        inputFile2->Close();
        return;
    }
    ...
    TH1F *hPolarAngle = new TH1F("hPolarAngle", "Количество кандидатов в #pi^{0} от полярного угла", 36, 0, 180);
    TH1F *hAzimuthalAngle = new TH1F("hAzimuthalAngle", "Количество кандидатов в #pi^{0} от азимутального угла", 36, -180, 180);
    ...
    std::cout << "Дисперсия для полярного угла: " << varPolar << std::endl;
    std::cout << "Дисперсия для азимутального угла: " << varAzimuth << std::endl;
